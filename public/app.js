...
function renderScooterItem(s) {
  const engineCut = s.engine_cut === 1;
  return `
    <div class="scooter-item ${s.inside_zone ? '' : 'outside'} ${engineCut ? 'killed' : ''}" id="si-${s.id}" onclick="focusScooter('${s.id}')">
      <div class="scooter-item-header">
        ...
        <div class="scooter-badges">
          ...
          ${engineCut ? '<span class="badge badge-killed">DURDURULDU</span>' : ''}
        </div>
      </div>
      <div class="scooter-item-footer">
        ...
        <div class="scooter-actions" onclick="event.stopPropagation()">
          ${engineCut
            ? `<button class="btn-restore" onclick="relayScooter('${s.id}', 'restore')">▶ Başlat</button>`
            : `<button class="btn-kill" onclick="relayScooter('${s.id}', 'cut')">⬛ Durdur</button>`
          }
          <button class="btn-delete" onclick="deleteScooter('${s.id}')">Sil</button>
        </div>
      </div>
    </div>
  `;
...
async function relayScooter(id, action) {
  const label = action === 'cut' ? 'durdurmak' : 'başlatmak';
  if (!confirm(`Bu scooteri ${label} istediğinizden emin misiniz?`)) return;

  try {
    const res = await fetch(`/api/scooters/${id}/relay`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (res.ok) {
      showToast(action === 'cut' ? '🔴 Scooter motoru durduruldu' : '🟢 Scooter motoru başlatıldı', action === 'cut' ? 'danger' : 'success');
    } else {
      showToast('⚠️ ' + (data.error || 'Komut gönderilemedi'), 'danger');
    }
  } catch (e) {
